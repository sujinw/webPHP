...
if(file_exists(KF_ROOT_PATH.'install.lock')) {xit('您已经安装过GZCMS,如果需要重新安装，请删除 install.lock 文件！');
...
switch($step)
{
	...
	case '3': //检测目录属性
		...
		foreach($files as $_k => $file) {
			...
			// echo $no_writablefile;die;
			// $filesmod[$_k]['no_writablefile'] = $no_writablefile;
		}
	...
	case '5': //安装详细过程
		...
		break;
	case '6'://完成安装
		file_put_contents(KF_ROOT_PATH.'install.lock','');
	...
	case 'installmodule': //执行SQL
		...
		if($module == 'admin') {
			...
			$in_config = array(
				'DB_CHARSET'	=>	str_replace("-","",$dbcharset),
				'DB_HOST'		=>	$dbhost,
				'DB_PORT'		=>  3306,
				'DB_USER'		=> 	$dbuser,
				'DB_PASSWORD'	=> 	$dbpass,
			...
			$in_config = "if(MYPHP_PATH)exit('没有权限访问');/n return ".var_export($in_config,true).";?>";
			$cache_file_path =KF_ROOT_PATH. "application/caches/config.php";
			write_static_cache_install($cache_file_path, $in_config, 1);
			...
			if(file_exists(KF_ROOT_PATH."install/main/".$dbfile)) {
				...
				$email = trim($email);
				$upsql = "UPDATE `".$pre."user` (`username`,`password`) VALUES('".$username."','".$password_md5."')";
				// echo $upsql;die;
				mysql_query($upsql);
				// 设置默认站点1域名
				...
				// _sql_execute("UPDATE `".$pre."config` SET value='".$sys_protocal."' WHERE name='site_domain';");
				// _sql_execute("UPDATE `".$pre."config` SET value='".$_PHP_SELF."' WHERE name='site_dir';");
			} else {
				echo '2';//数据库文件不存在
				exit;
			}
	...
	case 'testdata'://安装测试数据	
		$lnk = @mysql_connect($_GET['dbhost'], $_GET['dbuser'], $_GET['dbpass']) or die ('Not connected : ' . mysql_error());
		$version = @mysql_get_server_info($lnk);			
		if($version > '4.1' && $_GET['dbcharset']) {
			mysql_query("SET NAMES '".str_replace("-","",$_GET['dbcharset'])."'");
		}			
		if($version > '5.0') {
			mysql_query("SET sql_mode=''");
		}			
		mysql_select_db($_GET['dbname']);
		if(file_exists(KF_ROOT_PATH."install/main/gzcms_data.sql")){
			$sql = file_get_contents(KF_ROOT_PATH."install/main/gzcms_data.sql");
			_sql_execute($sql);
			//解压测试数据包
			if(file_exists(KF_ROOT_PATH."install/main/Upload.zip")){
		        include_once KF_ROOT_PATH."MyPHP/Extends/Tool/zip.class.php";
		        $archive  = new PHPZip();
		        $zipfile   = "main/Upload.zip";
		        // echo $zipfile;die;
		        $savepath  = KF_ROOT_PATH;
	        	...
	        	__fujian_del(KF_ROOT_PATH.'install/main/Upload.zip');	
	      	}
		}
		break;	
...
function _sql_split($sql,$r_tablepre = '',$s_tablepre='gz_') {
