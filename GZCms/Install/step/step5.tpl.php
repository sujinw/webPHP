...
var dbhost = '<?php echo $_POST["DB_HOST"]?>';
var dbuser = '<?php echo $_POST["DB_USER"]?>';
var dbpass = '<?php echo $_POST["DB_PASSWORD"]?>';
var dbname = '<?php echo $_POST["DB_DATABASE"]?>';
var pre = '<?php echo $DB_PREFIX?>';
var dbcharset = '<?php echo $_POST["DB_CHARSET"]?>';
var username = '<?php echo $_POST["username"]?>';
var password = '<?php echo $_POST["password"]?>';
var email = '<?php echo $email?>';
var ftp_user = '<?php echo $_POST["DB_USER"]?>';
function reloads() {
	...
	$.ajax({
		   type: "POST",
		   url: 'install.php',
		   data: "step=installmodule&module="+m_d[n]+"&dbhost="+dbhost+"&dbuser="+dbuser+"&dbpass="+dbpass+"&dbname="+dbname+"&pre="+pre+"&dbcharset="+dbcharset+"&username="+username+"&password="+password+"&email="+email+"&ftp_user="+ftp_user+"&t="+Math.random()*5,
			   ...
			   if(msg==1) {
				   ...
				   $('#installmessage').append(setting[m_d[n]] + msg + "<img src='images/correct.gif' /><br>");				   
					...
					if(n < m_d.length) {
						...
						if(testdata == 1) {
							$('#hiddenop').load("install.php?step=testdata&dbhost="+dbhost+"&dbuser="+dbuser+"&dbpass="+dbpass+"&dbname="+dbname+"&pre="+pre+"&dbcharset="+dbcharset+"&sid="+Math.random()*5+"'");
