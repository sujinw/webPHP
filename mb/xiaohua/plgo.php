...
@$sql="select * from pl where `classify`='xiaohua' and `uid`='$namee' and content='$content'";
$query=mysql_query($sql);
@$articletext=mysql_num_rows($query);
if($articletext>0 or $content==""){
$content="重复";
}else{
$sql="insert into `pl` (uid,name,classify,title,content) values ('{$namee}','$nameuser','xiaohua','{$time}','{$content}');";
$res = mysql_query($sql);
}
