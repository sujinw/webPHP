function addTansition(id, obj, opts, eopts){
	var _obj = document.getElementById(id),
		top = _obj.getBoundingClientRect().top, //元素顶端到可见区域顶端的距离
		se = document.documentElement.clientHeight //浏览器可见区域高度。

		console.log(se+"-----"+ top)

	if(top <= se ) {
		$$(obj).transition(opts, 1000, "easeInQuert");
	}else{
		$$(obj).transition(eopts, 1000, "in-out");
	}
};
function $$(obj){
