...
$scid=@$_GET['scid'];
$postid=@$_POST['scid'];
echo '<div class="nav" id="file"><a href="index.php" class="a">后台首页</a><a href="hy.php" class="a">会员管理</a><a href="article.php" class="b">文章管理</a><a href="ly.php" class="a">留言管理</a></div>';
if($scid){
echo '<div id="d">
...
if($postid){
$exec="delete from article where id=$postid";
...
$page=trim(@$_GET['page']);
$sql="select * from article where id and sh!=0";
$res=@mysql_query($sql);
...
$sql="select * from article where id and sh!=0 limit $kaishi,$pagesize";
$res=mysql_query($sql);
while($row=mysql_fetch_array($res)){
echo '<a href="hy.php?hyid='.$row['uid'].'"><li id="li">'.$row['name'].':'.$row['title'].'<a href="?scid='.$row['id'].'" id="yb">删除</a></li></a>';
}
if($pages<$page){
header("location:../");
}
...
echo "<a href='?id=$id&page=$pagej' id='nava'>第{$i}页</a>";
}
}
}
