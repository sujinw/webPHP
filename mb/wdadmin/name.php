...
ul{padding:0px;margin:0px;color:red}
...
$scid=@$_GET['scid'];
$postid=@$_POST['scid'];
echo '<div class="nav" id="file"><a href="index.php" class="a">后台首页</a><a href="hy.php" class="a">会员管理</a><a href="article.php" class="b">会员管理</a><a href="ly.php" class="a">留言管理</a></div>';
if($scid){
echo '<div id="d">
...
if($postid){
$exec="delete from name where id=$postid";
...
echo "<ul><li>会员总数({$nums})人</li>";
$kaishi=($page)*$pagesize;
$sql="select * from name where id limit $kaishi,$pagesize";
$res=mysql_query($sql);
while($row=mysql_fetch_array($res)){
echo '<a href="hy.php?hyid='.$row['id'].'"><li id="li">'.$row['id'].'<a href="?scid='.$row['id'].'" id="yb">删除</a></li></a>';
}
if($pages<$page){
header("location:../");
}
