...
$sql="select * from `pl` where `uid`='$id' and classify='bbs'";
...
$sql="select * from `pl` where `uid`='$id' and classify='bbs' order by `id` desc limit $pagex,$pagesize";
...
mysql_free_result($result);
$pages==0?$pages=1:"";
echo "<p id=\"page\">共{$nums}条评论-第{$pages}/{$page}页↑<p>";
}
?>