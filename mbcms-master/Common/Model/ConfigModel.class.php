...
class ConfigModel extends Model{
	...
	private function get_config_html($c){
		switch($c['show_type']){
			case 1://文本框
			 $html="<input type='text' class='form-control' name='{$c['id']}'  value='{$c['value']}' placeholder='{$c['title']}' required>";
			 break;
			case 2://单选框
				$option = $c['option'];
				$data = explode(',', $option);
				...
				foreach($data as $d){
			 	...
			 	break;
			 case 3://文本框 textarea
			 	$html="<textarea class='form-control' rows='3' name='{$c['id']}'>{$c['value']}</textarea>";
			 	break;
		}
		return $html;
	}
	...
	public function update_config(){
		foreach($_POST as $id=>$value){
			$this->where('id='.$id)->save(array('id'=>$id,'value'=>$value));
		}
		$config = $this->all();
		$d = array();
		foreach($config as $c){
