<?php
    class PHPZip
    {
        ...
        // $zipfile   = "ZIP压缩文件名";
        // $savepath  = "解压缩目录名";
        // 返回 array(文件夹数量,解压文件,失败)
        public function unZipfun($zipfile, $savepath)
        {
            $_array = $this->GetZipInnerFilesInfo($zipfile);
            $filecount = 0;
            $dircount  = 0;
            $failfiles = array();
            for($_j=0; $_j<count($_array); $_j++) {
                if($_array[$_j]['folder'] == 0){
                    if($this->unZip($zipfile, $savepath, $_j) > 0){
                        $filecount++;
                    }else{
                        $failfiles[] = $_array[$_j]['filename'];
                    }
                }else{
                    $dircount++;
                }
            }
            return array($dircount, $filecount, $failfiles);
        }
